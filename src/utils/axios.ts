...
axios.interceptors.response.use(
  (res: Type.Object) => {
    ...
    if (res.data.code === 401) {
      ...
      const pathstr: string =  window.location.href.split('#')[0]
      window.location.href = pathstr.endsWith('/') ? pathstr + '#/login' : pathstr + '/#/login'
      window.location.reload()
    }

    if (res.data.hasOwnProperty('result') && res.data.code === 200) {
