...
export const rndNum = (n: number, m: number) => {
  const random1 = Math.floor(Math.random() * (m - n + 1) + n)
  const random2 = Math.floor(Math.random() * (m - n + 1) + n)
  return random1 + '-' + random2
}
export function uuid() {
  const r1 = Math.random().toString(36).substr(2, 4).toUpperCase();
  const r2 = Math.random().toString(36).substr(2, 4).toUpperCase();
  return r1 + '-' + r2
}
export function findParentVm($this, componentname) {
  if ($this) {
    if ($this.comname === componentname) {
      return $this
    } else {
      return findParentVm($this.$parent, componentname)
    }
  } else {
    return false
  }
