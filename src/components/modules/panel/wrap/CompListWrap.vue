...
<template>
  <div class="wrap">
    ...
    <classHeader v-show="!currentCategory" :types="types" @select="selectTypes" :istext="true">
    ...
    <ul v-if="currentCategory" v-infinite-scroll="load" class="infinite-list" :infinite-scroll-distance="150" style="overflow: auto">
      <classHeader :is-back="true" @back="back"  :istext="true">{{ currentCategory.name }}</classHeader>
    ...
    async selectItem(item: any) {
      ...
      Array.isArray(group) &&
        group.forEach((element: any) => {
          ...
          element.parent = this.$store.getters.dPage.uuid
        })
