...
<template>
  <div class="wrap">
    ...
    <ul ref="listRef" v-infinite-scroll="load" class="infinite-list" :infinite-scroll-distance="150" style="overflow: auto">
      <img-water-fall :listData="list" @select="selectItem" @load="load" />
...
import { uuid, findParentVm } from '@/utils/utils.ts'
...
export default defineComponent({
  ...
  computed: {
    ...mapGetters(['tempEditing', 'dHistoryParams', 'dPageList']),
  },
  created() {
  ...
  methods: {
    ...
    if (rootVm && rootVm.imgCrop) {
      rootVm.imgCrop(false, true) 
    }
      ...
      result.tempid = item.id
      // if (result.page.uuid === '-1') { // 如果uuid是个-1，则处理成唯一值
        const curUuid = uuid()
        result.page.uuid = curUuid
        result.widgets.forEach(item => {
          item.parent = curUuid
          const childnewuuid = item.uuid + '&' + curUuid.split('-')[0]
          item.uuid = childnewuuid
        })
      // }
      const { page, widgets } = result
      console.log(widgets)
      this.$store.commit('setDPage', result)
