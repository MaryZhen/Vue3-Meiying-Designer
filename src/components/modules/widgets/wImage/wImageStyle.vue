...
<template>
  <div id="w-image-style">
    <el-collapse v-model="activeNames">
      ...
      <el-collapse-item title="设置" name="2">
        ...
        <div class="options">
          ...
          <!-- <el-button v-else plain type="primary" @click="imgCrop(true)" id="caijianbtn"><i class="icon sd-caijian" id="caijianicon" /> <span id="caijianspan">裁剪</span></el-button> -->
    ...
    <!-- mayz添加了一个display:none !important -->
    <inner-tool-bar v-show="innerElement.cropEdit" :style="toolBarStyle" style="display: none !important;">
      <number-slider v-model="innerElement.zoom" :is-wheel="true" class="inner-bar" label="缩放" labelWidth="40px" :step="0.01" :minValue="1" :maxValue="3" />
...
export default {
  ...
  computed: {
    ...mapGetters(['dActiveElement', 'dMoving', 'dWidgets', 'curIndex']),
  },
  watch: {
    dActiveElement: {
      handler(newValue, oldValue) {
        ...
        setTimeout(() => {
          if (newValue.mask) {
            this.imgCrop(true)
            const targetDiv = document.querySelector('.layer-no-hover')
            // this.oldZIndex  = targetDiv.style.zIndex
            this.$store.commit('changecurIndex')
            if (targetDiv) {
              targetDiv.style.zIndex = this.curIndex
            }
          }
        }, 0)
      },
      deep: true,
      immediate: true // mayz添加了一个immediate: true
    },
    innerElement: {
      handler(newValue, oldValue) {
        this.changeValue()
  ...
  created() {
    ...
    if (rootVm) {
      rootVm.imgCrop = (val, nochange) => {
        this.imgCrop(val, nochange)
      }
    }
  },
  beforeUnmount() {
    this.$store.commit('setCropUuid', -1)
    this.imgCrop(false)
  },
  methods: {
    ...
    changeValue() {
      ...
      for (let key in this.innerElement) {
        ...
        } else if (key !== 'cropEdit' && key !== 'record' && key !== 'imgwidth' && key !== 'imgheight' && this.innerElement[key] !== this.dActiveElement[key]) {
    ...
    imgCrop(val, nochange) {
      // TODO: 画布内图像裁剪
      const dom = document.getElementById(this.innerElement.uuid)
      if (dom) {
