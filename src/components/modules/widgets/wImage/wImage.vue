<template>
  ...
  >
    ...
    <div :style="{ transform: params.flip ? `rotate${params.flip}(180deg)` : undefined, borderRadius: params.radius + 'px', '-webkit-mask-image': `${params.mask ? `url('${params.mask}')` : undefined}` }" :class="['img__box', { mask: params.mask }]">
      ...
      <img v-else
        :ref="'targetimg' + params.uuid"
        :id="'target' + params.uuid"
        class="target"
        referrerpolicy="no-referrer"
        style="transform-origin: center"
        :style="{
          width: params.imgwidth ? params.imgwidth + 'px' : '100%',
          height: params.imgheight ? params.imgheight + 'px' : '100%',
          maxWidth: params.imgwidth ? params.imgwidth + 'px' : '100%'
        }"
        :src="params.imgUrl"
      />
    </div>
...
export default {
  ...
  computed: {
    ...
    isMask() {
      const res = this.params.mask && this.dDropOverUuid === this.params.uuid
      const target = document.getElementById(this.params.uuid)
      if (res) {
        if (target) {
        ...
        if (this.params.mask && target) {
          target.style.zIndex = this.oldZIndex
        }
      }
      return res
    },
  ...
  methods: {
    ...
    touchstart(e) {
      console.info('touchstart999999999999')
    ...
    move(payload) {
      if (payload) {
        ...
        const target = document.getElementById(this.params.uuid)
        const draggable = document.getElementById('target' + this.params.uuid)
        const zoom = parseFloat(this.params.zoom)
        const A = target.clientWidth
        const B = draggable.clientWidth
        const A1 = target.clientHeight
        const B1 = draggable.clientHeight
        const maxLeft = B * (zoom - 1) / 2
        const minLeft = A - B * (zoom + 1) /2
        const maxTop = B1 * (zoom - 1) / 2
        const minTop = A1 - B1 * (zoom + 1) /2
        if (left > maxLeft) {
          left = maxLeft
        }
        if (left < minLeft) {
    ...
    updateRecord() {
      ...
      // 获取点位
      if (!this.isDraw) {
        const read = new PointImg(this.$refs['targetimg' + this.params.uuid])
      }
    },
    setTransform(attrName, value) {
      ...
      this.params.transform && (this.$refs['targetimg' + this.params.uuid].style.transform = this.params.transform)
    },
