...
export default {
  ...
  mounted() {
    if (this.isWheel) {
      const divObj = document.getElementById('page-design')
      divObj.addEventListener('wheel', (event) => {
        this.wheelFn(event)
  ...
  methods: {
    ...
    wheelFn(event) {
      if (this.$store.state.design.showRotatable) {
        return false
      }
      if (event.deltaY > 0) {
        this.innerValue -= 0.2; // 缩小
      } else if (event.deltaY < 0) {
        this.innerValue += 0.2; // 放大
      }
    }
  },
}
