...
export default {
  ...
  addWidget(store: any, setting: any) {
    setting.uuid = nanoid()
    const newuuid = uuid()
    if (store.state.dPage.uuid === -1) {
      store.state.dPage.uuid = newuuid
    }
    setting.parent = store.state.dPage.uuid
    store.state.dWidgets.push(setting)
    const dPageList = store.state.dPageList
    const dPage = store.state.dPage
    if (dPageList && dPage) { // 处理上传psd后的widgets
      if (dPageList.length === 1 && !dPageList[0].page) {
        dPageList[0].page = dPage
        dPageList[0].widgets = []
      }
      const targetPage = dPageList.find(v => v.page.uuid === dPage.uuid)
      targetPage?.widgets?.push(setting)
    }
    const len = store.state.dWidgets.length
  ...
  addGroup(store: any, group: Type.Object) {
    ...
    if (dPageList && dPage) {
      targetPage = dPageList.find(v => v.page.uuid === dPage.uuid)
    }
    group.forEach((item: any) => {
  ...
  deleteWidget(store: any) {
    ...
    if (selectWidgets.length !== 0) {
      for (let i = 0; i < selectWidgets.length; ++i) {
        ...
        if (dPageList && dPage) {
            const targetPage = dPageList.find(v => v.page.uuid === dPage.uuid)
            targetPage?.widgets?.splice(index, 1)
        }
        try {
          // 清除掉可能存在的选中框
      ...
      if (activeElement.isContainer) {
      ...
      } else if (!activeElement.cusid) { // 如果选择的不是page
        // for (let i = widgets.length - 1; i >= 0; --i) {
        ...
        //     widget.parent = store.state.dPage.uuid
        //   }
        //   count = 0
        // }
      }
    ...
    if (!store.state.dActiveElement.cusid) {
  ...
  // TODO: 选中元件与取消选中
  selectWidget(store, { uuid, olduuid }) {
    let oldUuid = olduuid 
    if (!olduuid) {
      oldUuid = uuid
    }
    ...
    const selectWidgets = store.state.dSelectWidgets
    const widget = store.state.dWidgets.find((item) => item.uuid === uuid) || store.state.dPage
    if (alt) {
      if (widget.parent === uuid) {
        // && !widget.isContainer
        if (selectWidgets.length === 0) {
  ...
  realCombined(store: any) {
    ...
    if (selectWidgets.length > 1) {
      ...
      if (group.parent && group.parent === '-1') {
        group.parent = store.state.dPage.uuid
      }
      group.uuid = nanoid()
      widgets.push(group)
      ...
      // 修改对应的dPageList的widgets
      const dPageList = store.state.dPageList
      const targetPage = dPageList.find(v => v.page.uuid === store.state.dPage.uuid)
      if (targetPage) {
        targetPage.widgets = widgets
      }
  ...
  initGroupJson(store, json) {
    if (json && json.parent && store.state.dPage) {
      json.parent = store.state.dPage.uuid
    }
  ...
  ungroup(store, uuid) {
    ...
    const parentId = targetWidget.parent
    widgets.splice(index, 1)
    const len = widgets.length

    for (let i = 0; i < len; ++i) {
      if (widgets[i].parent === uuid) {
        widgets[i].parent = parentId
  ...
  setUserInfoAsync(store: any, data: any) {
