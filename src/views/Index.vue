<template>
  <div id="page-design-index" ref="pageDesignIndex" class="page-design-bg-color">
    ...
    <div class="page-design-index-wrap">
      ...
      <design-board ref="designboard" class="page-design-wrap" pageDesignCanvasId="page-design-canvas">
...
export default defineComponent({
  ...
  setup() {
    ...
    const state = reactive({
      ...
      comname: 'design-container', // 用于找组件
      timer: null
    })
    // const draw = () => {
    //   state.openDraw = true
  ...
  computed: {
    ...mapGetters(['dActiveElement', 'dHistoryParams', 'dCopyElement', 'dPage', 'dZoom', 'dAltDown', 'dWidgets']),
    undoable() {
  ...
  methods: {
    ...
    async clickListener(e) {
      ...
      const target:any = await getTarget(e.target)
      let iscutBtn = target.id === 'caijianbtn' || target.id === 'caijianicon' || target.id === 'caijianspan'
      
      const ismask = target.getAttribute('data-mask')
      const uuid = target.getAttribute('data-uuid') // 设置选中元素
      const type = target.getAttribute('data-type')
      if ((ismask && uuid === this.dActiveElement.uuid) || iscutBtn) {
        // 如果是本身，则取消
      } else {
        if (this.imgCrop) {
          this.imgCrop(false)
