<template>
  <div id="page-design-index" ref="pageDesignIndex">
    <div class="page-design-index-wrap canvasCut">
      <design-board :isdesign="false" class="page-design-wrap fixed-canvas" pageDesignCanvasId="page-design-canvas"></design-board>
    </div>
    <!-- 缩放控制 -->
    <zoom-control />
  </div>
  ...
  methods: {
    ...
    async load() {
      ...
      if (id || tempid) {
        ...
        const { data, width, height } = await api.home.gettemptempdetail({ id})
        const content = JSON.parse(data)
        const widgets = content.widgets

        this.$store.commit('setDPage', content)
