...
module.exports = {
  ...
  async screenshots(req: any, res: any) {
    ...
    const url = (screenshot_url || drawLink)
    id = id || tempid
    const path = filePath + `${id}-screenshot.png`
    ...
    if (id && width && height) {
      ...
      queueRun(saveScreenshot, targetUrl, { width, height, path, thumbPath, size, quality })
        .then(async () => {
          ...
          const imgPath: any = path || thumbPath
          const imageBuffer = fs.readFileSync(imgPath);
          // 将图片转换为base64
          const base64Image = imageBuffer.toString('base64');
          // console.log('获取到的base64是======', base64Image);
          const fixstr = imgPath.indexOf('.png') > -1 ? 'data:image/png;base64,' : 'data:image/jpeg;base64,'

          const baseStr = fixstr + base64Image
          // type === 'file' ? res.sendFile(path) : res.sendFile(thumbPath)
